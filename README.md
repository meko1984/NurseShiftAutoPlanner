...
「管理者支援システム Ver0.6」は、看護師などの勤務表作成を支援するローカルWebアプリです。勤務を自動で完成させるソフトではなく、勤務記号や登録済みパターンを配置しながら、勤務表のたたき台を作るためのツールです。
...
- 上部ヘッダー：アプリ名、評点、保存状態、月変更、シフト表クリア、初期化
- 勤務表エリア：行事予定、日付、曜日、スタッフ別勤務表、日別集計、警告行
- 勤務表操作エリア：自動調整、ハイライト色、勤務入力／希望入力の切り替え
- 勤務表下の設定エリア：パターン管理と勤務形態設定、その下にスタッフ管理
- 右側パネル：個人集計、NGペア、ルール設定
...
スタッフ管理で「自動調整」がOFFのスタッフは、自動調整の対象外です。ただし、手入力された勤務は集計や警告には含まれます。
...
「スタッフ管理」では、スタッフの追加、削除、P値、個人上限、自動配置・自動調整の対象設定を変更できます。

「自動配置」と「自動調整」はスタッフごとに別々にON/OFFできます。OFFにした処理では、そのスタッフの勤務は変更されません。

どちらをOFFにしても、勤務表上には表示され、手入力、希望入力、集計、警告、評点には通常通り含まれます。

スタッフごとに、遅出・入の月間回数と連勤日数の上限を設定できます。初期値は遅出4回、入4回、連勤6日です。上限は自動配置を禁止する条件ではなく、軽い評価要素として扱われます。超過した場合は個人集計の危険色、警告、評点、評点内訳に反映され、自動調整の改善候補になります。

## 勤務形態設定

右側の「勤務形態設定」では、病棟独自の勤務を追加・編集できます。設定できる内容は、記号、名称、区分、色、勤務／休み扱い、各集計への算入、Power・連勤への算入、自動配置での使用、翌日に必要な勤務、翌日に禁止する勤務、希望休前日の配置禁止です。

初期状態では `日・遅・入・明・休・有・夏・冬` が登録されています。追加した勤務形態は、勤務セルの選択画面、希望入力、パターン編集、凡例へ反映されます。使用中の勤務形態は、勤務表やパターンの意味が失われないよう削除できません。

勤務形態の記号を編集すると、保存済みの勤務・希望・パターン内の記号も新しい記号へ引き継がれます。勤務形態の設定とスタッフ個別設定は自動保存され、「シフト表クリア」では残り、「初期化」で初期値へ戻ります。
...
個人集計も0の場合は `0` と表示されます。公休数はルール設定の公休目標日数と比較して色が付きます。遅・入がスタッフ別上限を超えた場合は、その集計セルが危険色になります。明は勤務表と日別集計では使用しますが、個人集計には表示しません。
